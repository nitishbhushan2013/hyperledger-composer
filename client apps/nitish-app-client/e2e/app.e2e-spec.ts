...
describe('Starting tests for nitish-app-client', function() {
  ...
  it('website title should be nitish-app-client', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('nitish-app-client');
    })
  });

  it('network-name should be nitish-bna@0.0.2',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('nitish-bna@0.0.2.bna');
    });
  });

  it('navbar-brand should be nitish-app-client',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('nitish-app-client');
    });
  });
